...
window.PEERLEDGER_CLAIMS = [
  {
    ...
    slaHours: 6,
    priority: "Normal",
    type: "released_without_payment",
    typeLabel: "Released without payment",
    filedBy: "seller",
    order: {
  ...
  },
  {
    ...
    slaHours: 6,
    priority: "High",
    type: "cancelled_but_paid",
    typeLabel: "Cancelled but paid",
    filedBy: "buyer",
    order: {
  ...
  },
  {
    ...
    slaHours: 12,
    priority: "High",
    type: "bank_account_frozen",
    typeLabel: "Bank account frozen",
    filedBy: "seller",
    order: {
...
(function () {
  ...
  /* Numbered here rather than in the literal above, so adding a fixture
     never means renumbering the ones around it. Every appeal starts in the
     operator's task pool: that is where work is distributed to, and an
     appeal only leaves it by being handled. */
  window.PEERLEDGER_CLAIMS.forEach(function (c, i) {
    c.id = "Dispute" + String(i + 1).padStart(2, "0");
  });
